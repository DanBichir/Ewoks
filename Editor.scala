...
class Editor {
    ...
    /** Command: Transpose the characters on left and right of the cursor */
    def transposeCommand(): Boolean = {
	var p = ed.point

	//Tests for margins of file
	if (p == 0) 		{ beep(); return false }
	if (p == ed.length) 	{ beep(); return false }
...
object Editor {
	...
	Display.CTRLHOME  -> (_.moveCommand(HOME)),
	Display.CTRLEND   -> (_.moveCommand(END)),
